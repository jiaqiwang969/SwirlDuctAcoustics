...
%% 生成网格矩阵
    [theta,rho,z]=meshgrid(linspace(0,2*pi,thetaNumber),r',x_pole); %生成极坐标网格
...
% 导入数据，对网格赋值
 ...
 %%
     ...
     tsignal.cubes(solutime).v(10,:,:,:)=real(GGwArray*exp(-sqrt(-1)*w*t(solutime)));%可以为三维输入，p只有一维
     tsignal.cubes(solutime).v(11,:,:,:)=real(TTm1Array*exp(-sqrt(-1)*w*t(solutime)));%0度
     tsignal.cubes(solutime).v(12,:,:,:)=real(TTm2Array*exp(-sqrt(-1)*w*t(solutime)));%45度
     tsignal.cubes(solutime).v(13,:,:,:)=real(TTm3Array*exp(-sqrt(-1)*w*t(solutime)));%90度
...
%NAME10 = ['GreenSwirl-Mr_3D','-',date];  %存储文件夹   ß
