...
if length(k_nm)&length(x0)~=0 %Wave number may be empty for high circumferential modes
    for kk=1:length(k_nm)
        ...
        C_rkw2=(U_theta-w^2*Phi^2)^2*(Phi^2/c02-(k_nm(kk)/w)^2-(m/w)^2/xr^2)+Ri*(U_theta-w^2*Phi^2)...
            *(Ri+Phi*(1/xr-diff(rou0)/rou0))+Ri*(diff(Phi*(w^2*Phi^2-U_theta)))-Phi*(w^2*Phi^2-U_theta)*(diff(Ri));%(James-3.1.21)
        f1=(2*m/w*U_theta(1)/Phi(1)+U_theta(1)^2/c02(1))...
            +(Boundary==2||Boundary==4)*sqrt(-1)*rou0(1)/z_t*(w*Phi(1)^2-U_theta(1)/w);%(James-3.1.22)&&(James-3.1.17)
        f2=(2*m/w*U_theta(Ratio)/Phi(Ratio)/Ratio^2+U_theta(Ratio)^2/c02(Ratio))/Ratio...
            -(Boundary==3||Boundary==4)*sqrt(-1)*rou0(1)/z_h*(w*Phi(Ratio)^2-U_theta(Ratio)/w);%(James-3.1.23)
        L=chebop(Ratio, 1);
        ...
        f1_k=-2*m/w*U_theta(1)/Phi(1)^2*Phi_k(1)...
            +(Boundary==2||Boundary==4)*sqrt(-1)*rou0(1)/z_t*w*2*Phi(1)*Phi_k(1);%(James-3.1.22)&&(James-3.1.17)
        f2_k=-2*m/w*U_theta(Ratio)/Ratio^2/Phi(Ratio)^2*Phi_k(Ratio)...
            -(Boundary==2||Boundary==4)*sqrt(-1)*rou0(Ratio)/z_h*w*2*Phi(Ratio)*Phi_k(Ratio);%(James-3.1.23)&&(James-3.1.17)
        Lk1=L;Lk2=L;
        ...
        R01mk=(diff(xr*M_theta)*m/xr^2+diff(Mx)*k_nm(kk))*Dmk-2*(diff((Mx)*k_nm(kk))...
            +diff(M_theta/xr)*m)*Amk^2-diff(2*M_theta/xr^2*diff(xr*M_theta))*Amk;%(Posson-JFM-2013-C1)未包括偏导数部分
        ...
        for xk0=x0
            ...
            Tgm1{kk,kkk}=sqrt(-1)*(Tmk_1*G_m{kk,kkk}+diAxis1(2)*(2*M_theta/xr*Amk*Dmk*diff(G_m{kk,kkk})))...
                *exp(sqrt(-1)*k_nm(kk)*xk0); %(Posson2012AAIA-44b),关于变量r0的函数，xd0=0
            Tgm2{kk,kkk}=sqrt(-1)*(Tmk_2*G_m{kk,kkk}+diAxis2(2)*(2*M_theta/xr*Amk*Dmk*diff(G_m{kk,kkk})))...
                *exp(sqrt(-1)*k_nm(kk)*xk0); %(Posson2012AAIA-44b),关于变量r0的函数，xd0=0
            Tgm3{kk,kkk}=sqrt(-1)*(Tmk_3*G_m{kk,kkk}+diAxis3(2)*(2*M_theta/xr*Amk*Dmk*diff(G_m{kk,kkk})))...
                *exp(sqrt(-1)*k_nm(kk)*xk0); %(Posson2012AAIA-44b),关于变量r0的函数，xd0=0
