  % 保存图像
  function [h1,h2]=saveFun(r,m,Pw,Gw,Pwn,Gwn,Tr,Omag,save_directory,Boundary,Type)  
    [rho,theta]=meshgrid(r',linspace(0,2*pi,360)); %生成极坐标网格
    [x,y]=pol2cart(theta,rho); %将极坐标网格转化为直角坐标网格
    h1=figure('Visible', 'on');
    subplot(2,2,1);contour(x,y,real(Pw)','Fill','on');axis equal
    colormap Parula;colorbar('location','EastOutside');
    title({['Plot of Real(Pw)'];[char(Type(Boundary)),'-Tr=',num2str(Tr),'-Omag=',num2str(Omag)]})
    subplot(2,2,2);contour(x,y,imag(Pw)','Fill','on');axis equal
    colormap Parula;colorbar('location','EastOutside');
    title({['Plot of Imag(Pw)'];[char(Type(Boundary)),'-Tr=',num2str(Tr),'-Omag=',num2str(Omag)]})
    subplot(2,2,[3,4]);
    bar(m,Pwn);set(gca,'YMinorTick','on','YScale','log');
    %set(gca,'XTick',m);
    set(gca,'Ygrid','on') 
    title({['Plot of maxPw for each azimuthal number n'];[char(Type(Boundary)),'-Tr=',num2str(Tr),'-Omag=',num2str(Omag)]},'FontSize',14)
    ...
    saveas(h1,[save_directory,'\','Pw',char(Type(Boundary)),'-Tr=',num2str(Tr),'-Omag=',num2str(Omag),'.png'])
    saveas(h2,[save_directory,'\','Gw',char(Type(Boundary)),'-Tr=',num2str(Tr),'-Omag=',num2str(Omag),'.png'])
