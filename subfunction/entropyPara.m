...
switch  Entropy
    case {0} %均熵
        ...
        rou0=c02.^(1/(gama-1)); P0=1/gama*(c02.^(gama/(gama-1)));%均熵假设(非均熵多一个参数beita-3.36)
    ...
    case {1}  %by Guan 
        ...
        P0=1/gama*(rou0.^gama)./(r.^beta);   %非均熵假设-3.36
        c02=gama*P0./rou0;
    case {2}  %by James   
        x = chebfun('x',[Ratio 1]);
        f=cumsum(x^(beta/Cp)*(Tr/x+Omag*x)^2/x);
        c02 = 1*x^(-beta/Cp)-(gama-1)*x^(-beta/Cp)*(f-f(1));c02=c02(r);%(James-2.1.19)
    ...
    case {3}  %for James  将gama->1 from Guan 参考熵为0=ln（1）
        ...
        rou0=EntropyFun\0;rou0=rou0(r);
        P0=(rou0.^gama)./(r.^beta);   %非均熵假设-3.36
        c02=gama*P0./rou0;
